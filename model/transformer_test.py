...
class Transformer:
    ...
    def compile(self):
        ...
        # Take only the last time step: (batch, height, width, channels)
        last_surface = inputs[:, -1, :, :, :]

        # Reshape (batch, height * width, channels)
        x = tf.keras.layers.Reshape((height * width, channels))(last_surface)

        # Project to embedding dimension
        x = tf.keras.layers.Dense(64)(x)

        # Attention mask: not needed for spatial attention usually (optional if you want it)
        ...
        x = tf.keras.layers.Dense(1)(x)  # one output per patch

        # Reshape back to (height, width, 1)
        outputs = tf.keras.layers.Reshape((height, width, 1))(x)
