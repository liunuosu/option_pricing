...
def write_results(folder_path, ivrmse, r_oos, ivrmse_h, r_oos_h, surface, \
                   surface_pred, covariate_columns, option_type, smooth, window_size, h_step, note):
    ...
    for i in covariate_columns:
        cov = cov+ "_" +i
    np.save(ivrmse_path / f"{option_type}_smooth_{smooth}_ws_{window_size}_h_{h_step}{cov}_{note}.npy", ivrmse)
    np.save(r_oos_path / f"{option_type}_smooth_{smooth}_ws_{window_size}_h_{h_step}{cov}_{note}.npy", r_oos)
    np.save(ivrmse_h_path / f"{option_type}_smooth_{smooth}_ws_{window_size}_h_{h_step}{cov}_{note}.npy", ivrmse_h)
    np.save(r_oos_h_path / f"{option_type}_smooth_{smooth}_ws_{window_size}_h_{h_step}{cov}_{note}.npy", r_oos_h)
    np.save(surface_path/ f"{option_type}_smooth_{smooth}_ws_{window_size}_h_{h_step}{cov}_{note}.npy", surface)
    np.save(surface_pred_path / f"{option_type}_smooth_{smooth}_ws_{window_size}_h_{h_step}{cov}_{note}.npy", surface_pred)